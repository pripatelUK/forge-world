...
export const signUserOp = async (userOpHash: string, passkeyIdHex: string, signatureResponse: any): Promise<string> => {
...
export const signUserOpWithCreate = async (userOpHash: string, login: string, passkey: any): Promise<string> => {
  console.log({ userOpHash });
  const challenge = Buffer.from(userOpHash.slice(2), 'hex');
  ...
  //     name: `${login} ${new Date().toLocaleDateString('en-GB', {
  //       day: '2-digit',
  //       month: '2-digit',
  //       year: 'numeric',
  //       hour: '2-digit',
  ...
  console.log(login)
  await AsyncStorage.setItem(`loginID`, login);
  ...
  const { data: loginServiceData } = await axios.request({
    method: 'POST',
    url: `http://192.168.43.114:${VITE_LOGIN_SERVICE_PORT}/login`,
    data: {
      login,
      credId,
      pubKeyCoordinates,
